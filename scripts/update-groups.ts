...
export const countryGroups: CountryGroup[] = ${JSON.stringify(this.staticGroups, null, 2)
        .replace(/"POLITICAL_UNION"/g, 'GroupingType.PoliticalUnion')
        .replace(/"MILITARY_ALLIANCE"/g, 'GroupingType.MilitaryAlliance')
        .replace(/"ECONOMIC_UNION"/g, 'GroupingType.EconomicUnion')
        .replace(/"CONTINENT"/g, 'GroupingType.Continent')
        .replace(/"REGION"/g, 'GroupingType.Region')
        .replace(/"CUSTOMS_UNION"/g, 'GroupingType.CustomsUnion')
        .replace(/"CURRENCY_UNION"/g, 'GroupingType.CurrencyUnion')
        .replace(/"FREE_TRADE_AREA"/g, 'GroupingType.FreeTradeArea')
        .replace(/"CULTURAL_GROUP"/g, 'GroupingType.CulturalGroup')
        .replace(/"OTHER"/g, 'GroupingType.Other')};
`;
